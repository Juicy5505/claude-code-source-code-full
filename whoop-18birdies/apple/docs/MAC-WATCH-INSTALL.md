...
3a-pre. **"This app cannot be installed because its integrity could not be verified"** (on the Watch): the app REACHED the Watch — progress — but it was signed with a profile that does not include the Watch's device ID. This is the signature failure of the phone→Watch transfer path under a free Personal Team: the profile was minted for the iPhone build, and the Watch was never registered into it. The fix is the direct path below (`ship-to-watch.sh`, or scheme WhoopGolfWatch → the Watch as destination): building TO the Watch with automatic signing registers the Watch and mints a profile that covers it. If the same message appears after a DIRECT install, the developer certificate is not trusted yet — iPhone → Settings → General → VPN & Device Management → Trust, then reinstall.
